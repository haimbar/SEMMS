...
#' @param mincor The minimum correlation coefficient between pairs of putative variable, over which they are considered highly correlated. Default is 0.7.
...
#' \item{mincor}{The user's input for mincor above. Used as the correlation lockout threshold in GAMupdate regardless of initWithEdgeFinder.}
...
#' @return \item{mod}{The object returned from glm or lm.}
...
mds2D <- function(dist1) {
  ...
  K2.eigs <- eigen(K2)
  coord2D <- cbind(sqrt(abs(K2.eigs$values[1]))*K2.eigs$vectors[,1],
                   -sqrt(abs(K2.eigs$values[2]))*K2.eigs$vectors[,2])
...
readInputFile <- function(file, skip=0, header=TRUE, sep="\t", ycol, Zcols,
  ...
  if (length(grep("RData",file)) > 0) {
    ...
    stop(paste0("Unrecognized file format: ", file,
                ". Supported formats: .RData, .txt, .csv, .dat"))
  }
...
#' Ozone data
#'
#' The air-pollution data set analyzed in this case study was first introduced in (Breiman and Friedman 1985). It consists of daily measurements of ozone concentration levels in the Los Angeles basin, collected over 330 days in 1976. There are eight meteorological explanatory variables, plus the day of the year. Note that the response (column 2) has to be log transformed. The predictors appear in columns 3-11. We add an intercept, and construct the two-way interaction terms in the example below.
#'
#' @docType data
#' @keywords datasets
#' @name ozone
...
#' fittedGLM <- runLinearModel(dataYXZ,fittedSEMMS$gam.out$nn, "N")
#' plotMDS(dataYXZ, fittedSEMMS, fittedGLM, ttl="AR1 simulation")}
#' @format A 100 by 1001 data frame (100 observations: 1 response column followed by 1000 predictor columns).
NULL
