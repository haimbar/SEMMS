...
It provides code and sample output for simulated data and case studies, and documents the mixed-effects extension (`fitSEMMSmixed()`) introduced in version 0.4.0, which adds support for random intercepts and/or random slopes for **Gaussian responses**.
...
- Version 0.4.0 adds `fitSEMMSmixed()`, an alternating coordinate-ascent extension that supports random intercepts, random slopes, or both, via the `lme4` package. **This option is currently available for Gaussian responses only.** See the [Mixed-Effects Extension](#mixed-effects-extension) section below for details.
...
> **Note:** The mixed-effects extension is currently implemented for
> **Gaussian responses only**.  Support for Poisson and binomial responses
...
1. **Fixed-effects step.** Run `fitSEMMS()` on the RE-adjusted response
   $Y^* = Y - \hat{u}$, where $\hat{u}$ is the current vector of random-effect
   (BLUP) contributions.
2. **Random-effects step.** Refit an `lmer` model conditional on the currently
   selected fixed effects (using ML); extract the new BLUP contributions as the
   updated $\hat{u}$.

The loop is warm-started with a null `lmer` (intercept-only fixed part) and
stops when $\max|\Delta \hat{u}| <$ `outer_tol` (default $10^{-3}$).  A final
REML `lmer` with the selected predictors is returned as the reported model.
In practice the loop converges in 2–4 outer iterations.
...
### Example: simulated longitudinal data
